...
exports.run = async(client, message, args) => {
  ...
  if(panel) return message.channel.send(`Bu sunucuda panel zaten ayarlanmış! Devredışı bırakmak için;  \`${prefix}sunucupanel sil\``)
      ...
      message.channel.send(new Discord.MessageEmbed().setColor('RANDOM').setTitle('Sunucu Panel').setDescription('<a:RainbowOkGif:807560491713953821> Gerekli dosaylar kurulsun mu?.').setFooter('Onaylıyorsan 15 saniye içerisinde "evet" yazmalısın.'))
.then(() => {
